function results = evaluateWithCapture(code, options)
    ...
    arguments
        ...
        options.FigureVisibilityController(1, 1) mcpcoreserver.internal.display.FigureVisibilityController = mcpcoreserver.internal.display.DefaultFigureVisibilityController()
        options.OutputCapture(1, 1) mcpcoreserver.internal.capture.OutputCapture = mcpcoreserver.internal.capture.DefaultOutputCapture()
        options.CapturedEventProvider(1, 1) mcpcoreserver.internal.capture.CapturedEventProvider = mcpcoreserver.internal.capture.DefaultCapturedEventProvider()
        options.EventDisplayer(1, 1) mcpcoreserver.internal.display.console.EventDisplayer
        options.EventPoller(1, 1) mcpcoreserver.internal.capture.EventPoller
        options.OutputBuilder(1, 1) mcpcoreserver.internal.output.OutputBuilder = mcpcoreserver.internal.output.OutputBuilder()
    end
    ...
    if ~isfield(options, 'EventDisplayer')
        options.EventDisplayer = mcpcoreserver.internal.display.console.DefaultEventDisplayer();
    end

    if options.DisplayDuringExecution && ~isfield(options, 'EventPoller')
        options.EventPoller = mcpcoreserver.internal.capture.DefaultEventPoller( ...
    ...
    if ~isempty(userError)
        throwAsCaller(mcpcoreserver.internal.error.ExceptionReport.stripInternalFrames( ...
