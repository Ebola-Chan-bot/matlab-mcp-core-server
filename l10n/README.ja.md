...
1. 本サーバーは Model Context Protocol を使用するすべての AI アプリケーションをサポートしています。Claude Desktop 用に MATLAB MCP Core Server をセットアップする場合は、「[Claude Desktop](#claude-desktop)」の手順に進んでください。その他のアプリケーション用にサーバーをセットアップする場合は、以下の手順に従います。
